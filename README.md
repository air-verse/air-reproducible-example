...
- `windows-path-bug/`: **Windows-only:** Air fails to run binaries when path is provided via CLI flags with forward slashes (e.g., `--build.bin "bin/app.exe"`); config file works fine (reproduces air-verse/air#589).
